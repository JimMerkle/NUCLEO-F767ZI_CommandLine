...
#include "cl_freertos.h" // FreeRTOS interactive routines
#include "version.h"
...
const COMMAND_ITEM cmd_table[] = {
	...
	{"dump",      "dump <address> <optional byte count>",         2, cl_dump}, // hexdump.c
...
#endif // STM32F103xB
#ifdef STM32F446xx
	volatile uint8_t * p_id = (uint8_t *)0x1FFF7A10; // STM32-F446RE
#endif // STM32F446xx
...
//Address: 0x1FFFF7E0 (STM32-F103RB), or 0x1FF0F442 (STM32-F767ZI), or 0x1FFF7A22 (STM32-F446RE)
...
#endif // STM32F103xB
#ifdef STM32F446xx
	volatile uint16_t * p_k_bytes_flash = (uint16_t *)0x1FFF7A22; // STM32-F446RE
#endif // STM32F446xx
...
// Perform a timer5 test.  Timer5 is a 32-bit timer, counting microseconds
// Is the us timer tracking System Ticks?
// Add "HAL_TIM_Base_Start(&htim5); // Start the timer running", to end of MX_TIM5_Init(), after "USER CODE BEGIN TIM5_Init 2"
int cl_timer(void)
{
    printf("%s(), Timing HAL_Delay(50)\n",__func__);
    volatile TIM_TypeDef *TIMx = TIM5;
...
// Using a 32 bit timer spin-delay a quantity of micro-seconds (Timer 5 on STM32F446RE is a 32-bit timer)
...
uint32_t timer_delay_us(uint32_t delay_us)
{
    //printf("%s(%lu)\n",__func__,delay_us);
    volatile TIM_TypeDef *TIMx = TIM5; // Establish pointer to timer 5 registers
    uint32_t start_us = TIMx->CNT; // function entry count
    uint32_t delta;
    do {
